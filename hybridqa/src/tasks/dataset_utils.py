...
def convert_df_type(df: pd.DataFrame, lower_case=True):
    ...
    new_columns = []
    for _idx, header in enumerate(df.columns):
        if header == "":
            new_columns.append(
                "FilledColumnName"
    ...
    for col_i, one_col_values in enumerate(all_col_values):
        ...
        for _row_i, cell_value in enumerate(one_col_values):
            try:
                float(cell_value)
            except Exception:
...
def table_to_df(table: dict, add_row_id=True, normalize=True, lower_case=True):
    header, rows = table["header"], table["rows"]
