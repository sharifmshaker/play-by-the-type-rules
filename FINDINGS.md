...
(**Gemini 3.1 Flash-Lite**) against a local open-weight model (**Gemma E4B**, FP8),
...
text**, while the local open model is **~100–230× cheaper and several× faster**, and
fully reproducible, with no rate limits. Which one wins is a value judgment on that
tradeoff, not an open empirical question.
